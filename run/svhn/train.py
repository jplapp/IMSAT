...
def main():
	...
	import tools.svhn as svhn

	images_train, labels_train = svhn.get_data('train', max_num=72000)  # no train labels nowhere
	images_test, labels_test = svhn.get_data('test', max_num=20000)
	print(images_train.shape, images_test.shape, labels_test.shape, labels_train.shape)
    
    
    #scale the same way as mnist was scaled (in range 0 - 1)
	images_train = np.array(images_train, dtype=np.float32) / 256.
	images_test = np.array(images_test, dtype=np.float32) / 256.
    
	print(images_train.min(), images_train.max(), images_train.mean())
